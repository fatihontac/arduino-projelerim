...
void handleSMS(String sender, String smsText) {
  ...
  if (smsUpper == "AC") {
    ...
    durum += "\nSicaklik: ";
    durum += okundu ? String(sicaklik, 1) : String("Hata");
    durum += " C";
    durum += "\nNem: ";
    durum += okundu ? String(nem, 1) : String("Hata");
    durum += " %";
...
bool readDHTFresh(float &sicaklik, float &nem) {
  // DHT11 bazen ilk okumalarda takili veri verebilir, bu yuzden yeniden baslatip
  // art arda birkac kez okuyarak gecerli taze veri almaya calisiyoruz.
  dht.begin();
  delay(1500);
