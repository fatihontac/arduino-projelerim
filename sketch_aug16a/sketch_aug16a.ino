...
//const int64_t GROUP_CHAT_ID = -1229956787;
...
void setup() {
  ...
  secured_client.setInsecure();  // Sertifika doğrulaması devre dışı

  bot.setTelegramToken(BOT_TOKEN);
  bot.setUpdateTime(1000);
  bot.begin();  // Bot bağlantısını başlat
  Serial.println("Bot bağlantısı başlatıldı.");
...
void loop() {
  ...
  if (bot.getNewMessage(msg)) {
    if (msg.chatId != GROUP_CHAT_ID) return;  // Sadece grup mesajlarına yanıt ver
