...
void setup() {
  ...
  digitalWrite(RELAY_PIN, HIGH);  // Başlangıçta KAPALI
  ...
  Serial.println("RELAY_LOW  -> AÇIK (LOW)");
  Serial.println("RELAY_HIGH -> KAPALI (HIGH)");
...
void loop() {
  ...
  while (E22.available() > 1) {
    ...
    if (gelenMesaj == "RELAY_LOW") {
      ...
      relayState = true;
      lastSignalTime = currentTime;  // ZAMANI SIFIRLA!
    }
